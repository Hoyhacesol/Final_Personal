...
const ReadPage = () => {
  ...
  const [profileModal, setProfileModal] = useState(null); //주석: 닉네임클릭시 모달

  const { moveToList, moveToModify } = useCustomMove();
  const { user } = useAuth();
  const navigate = useNavigate();
  const { openMessengerWithUser } = useOutletContext();
        ...
        <div className="read_header">
          ...
          <div className="read_meta">
            <div className="read_info">
              <span>👤 {community.writerName || "탈퇴한사용자"}</span>
          ...
          {comments.map((c) => (
            <li key={c.cno} className="comment_item">
              <div className="comment_content">
                ...
                {/* 주석: 닉네임 클릭시 모달 */}
                <span className="comment_author_wrap">
                  <span
                    className="comment_author"
                    onClick={() => {
                      // 본인이면 모달안뜸
                      if (user?.mno === c.mno) return;
                      setProfileModal(c.cno);
                    }}
                  >
                    {c.writerName || "탈퇴한사용자"}
                  </span>
